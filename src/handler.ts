...
const dynamoOptions = isLocal
  ? { region: "ap-northeast-1", endpoint: "http://localhost:50000" }
...
export const getRemainingTasks: APIGatewayProxyHandler = async (
  event: GetRemainingTasksProp
) => {
...
export const postRemaingTasks: APIGatewayProxyHandler = async (
  event: PostProp
) => {
...
const updateRemainigTasks = async (event: PostProp) => {
  ...
  const result = getDefaultResult("Data is not put correctly")
...
export const getTimeLists: APIGatewayProxyHandler = async (
  event: GetTimeListsProp
) => {
...
export const postTimeLists: APIGatewayProxyHandler = async (
  event: PostProp
) => {
...
function getDefaultResult(
  message = "This is NOT correct data"
): APIGatewayProxyResult {
...
export const getSampleData: APIGatewayProxyHandler = async (
  event: GetSampleDataProp
) => {
  const sample =
    event.queryStringParameters.sample === undefined
      ? "nothing"
      : event.queryStringParameters.sample
  const result = {
    statusCode: 200,
    body: JSON.stringify({
